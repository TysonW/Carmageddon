...
CMotor::~CMotor()
{
	...
	gpioWrite(STBY,0);//Set Standby pin to low
}
...
void CMotor::MotorInitialise()
{
		...
		//initialise standby and set it high
		gpioSetMode(STBY, PI_OUTPUT);
		gpioWrite(STBY, PI_HIGH);
...
void CMotor::FORWARD(int spdL, int spdR)
{
	//set motor A(LEFT) to FWD
	gpioWrite(AIN1, PI_HIGH);
	gpioWrite(AIN2, PI_LOW);
	
	//set motor B(RIGHT) to FWD
	gpioWrite(BIN1, PI_HIGH);
	gpioWrite(BIN2, PI_LOW);
	
	//set motor A(LEFT) and B(RIGHT) to specified speed
	gpioPWM(PWMA,spdL+ 0.025);
	gpioPWM(PWMB,spdR );
