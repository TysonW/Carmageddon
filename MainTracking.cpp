...
void test_com()
{
  // Comport class
  Serial com;		
  com.open("COM8"); //Random port

  // TX and RX strings
  std::string tx_str = "G 1 0\n";
  std::string rx_str;
...
void do_image()
{
  ...
  for (int i = 0; i < 500; i++)
  {
    ...
    cv::Point center = cv::Point(im.size().width*rand() / RAND_MAX, im.size().height*rand() / RAND_MAX); //Point(x,y)
    cv::circle(im, center, radius, cv::Scalar(200, 200, 200), 1, cv::LINE_AA);//
    
    im.at<char>(i,i) = 255;
    
    cv::imshow("Image", im);  //imshow("name", matrix)
    cv::waitKey(1);
  }
}
